...
auto main(int argc, char **argv) -> int
{
    ...
    auto tc_register = toolchain->add_subcommand("register", "Register a toolchain in the cache");
    tc_register->add_option("-N,--name", toolchain_name_in, "Toolchain name")->required();
    tc_register->add_option("-T,--toolchain-path", toolchain_path_in, "Path to toolchain JSON")->required();
    ...
    tc_remove->add_option("-N,--name", toolchain_name_in, "Toolchain name")->required();

    auto tc_clear = toolchain->add_subcommand("clear", "Clear all cached toolchains");

    auto tc_show = toolchain->add_subcommand("show", "Show a cached toolchain's contents");
    tc_show->add_option("-N,--name", toolchain_name_in, "Toolchain name")->required();
    ...
    if (configure->parsed())
    {
        ...
        auto tc = cppbuild::get_toolchain(toolchain_name_in, self_path);
    ...
    else if (tc_register->parsed())
    {
        std::filesystem::path tc_path {std::filesystem::weakly_canonical(toolchain_path_in)};
        cppbuild::register_toolchain(toolchain_name_in, tc_path, self_path);
    }
    ...
    else if (tc_clear->parsed())
    {
        cppbuild::clear_toolchains(self_path);
    }
    else if (tc_show->parsed())
    {
        cppbuild::show_toolchain(toolchain_name_in, self_path);
    }
    return 0;
}
