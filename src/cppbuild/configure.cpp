...
export namespace cppbuild::configure
{
    ...
    auto conf(const conf_options &opts) -> int
    {
        types::toolchain tc{};
        umka::umka umka{};
        ...
        compdb::fill_module_names(graph, scanner::run_scan_deps(opts.build_dir, tc));
    ...
    struct reconf_options
    {
        public:
            const std::filesystem::path &build_dir;
            const std::filesystem::path &self_path;
    };
    auto reconf(const reconf_options &opts) -> int
    {
        ...
        compdb::fill_module_names(graph, scanner::run_scan_deps(opts.build_dir, cache.toolchain));
        const auto order = graaf::algorithm::dfs_topological_sort(graph);
        if (!order)
        {
